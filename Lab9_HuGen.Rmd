...
* Note the format of the daily report has changed several times, most recently on March 23rd. The column names are now different and additional data (more define state locations - county). Please adjust your scripts accordingly. 
...
report_03_11_2020 %>% 
  ...
  ggplot(aes(x = Confirmed,  y = reorder(Province_State, Confirmed))) + 
    geom_point() +
    ggtitle("Confirmed cases for each US State - March 11, 2020") +
    ylab("Country/Region") +
    xlab("Confirmed Cases")
```

If we try to graph by Country.Region there are entries that have the same character (e.g. China and US) so we need to summarise the data we are interested in graphing
...
report_03_11_2020 %>% 
  ...
  ggplot(aes(x = Deaths,  y = reorder(Country_Region, Deaths))) + 
    ...
    ggtitle("The 20 countries with the most reported COV19-related deaths - March 11, 2020") +
    ylab("Country/Region") +
    xlab("Deaths")
```
